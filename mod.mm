...
static NSString *const kClientId = @"00000000441cc96b"; // Nintendo Switch Client ID (Device Code Flow)

#pragma mark - Локальное хранилище данных авторизации
...
static OSStatus hooked_SecItemAdd(CFDictionaryRef attributes, CFTypeRef *result) {
    ...
    // При ошибке прав доступа (-34018) сохраняем запись в изолированный plist
...
#pragma mark - Device Code Flow (Вход через второе устройство)
...
static void PollForToken(NSString *deviceCode, NSInteger interval, UIAlertController *waitingAlert) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        while (!completed && waitingAlert.isBeingPresented) {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [waitingAlert dismissViewControllerAnimated:YES completion:^{
                                UIAlertController *successAlert = [UIAlertController alertControllerWithTitle:@"Успешно!"
                                    message:@"Вход выполнен. Перезапустите Minecraft, чтобы применить авторизацию."
    ...
    [[[NSURLSession sharedSession] dataTaskWithRequest:req completionHandler:^(NSData *data, NSURLResponse *res, NSError *err) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *message = [NSString stringWithFormat:
                @"1. Откройте на ПК или другом устройстве:\nmicrosoft.com/link\n\n"
                @"2. Введите код:\n%@\n\n"
                @"Не закрывайте это окно, игра автоматически ждет подтверждения.",
                userCode];
...
#pragma mark - Перехват кнопки входа через ASWebAuthenticationSession
